...
- **Prompt Management**: Centralized prompt library with versioning, A/B testing, and monitoring.

## Prompt Management Architecture

The system includes a sophisticated prompt management layer that sits between the Agent Orchestrator and the LLM Gateway:

### Prompt Library
A centralized service for storing, versioning, and managing prompt templates:

```mermaid
flowchart LR
    subgraph PromptLibrary["Prompt Library"]
        PT["Prompt<br/>Templates"]
        PV["Version<br/>Control"]
        MV["Model<br/>Variants"]
        VAR["Variable<br/>Substitution"]
    end
    
    subgraph Monitoring["Prompt Monitoring"]
        PM["Performance<br/>Metrics"]
        AB["A/B Test<br/>Results"]
        AL["Alerting"]
        QM["Quality<br/>Metrics"]
    end
    ...
    classDef library fill:#fef3c7,stroke:#f59e0b,color:#000;
    classDef monitor fill:#f3f4f6,stroke:#9ca3af,color:#000;
    
    class PT,PV,MV,VAR library;
    class PM,AB,AL,QM monitor;
```

### Key Features

**Prompt Library:**
- **Prompt versioning and rollback**: Track changes to prompts over time
- **Model-specific prompt variants**: Optimize prompts for different LLM providers (Ollama, OpenAI, Claude)
- **Performance-based prompt selection**: Choose prompts based on success rate and quality
- **Collaborative prompt editing**: Multiple team members can iterate on prompts
- **Variable substitution**: Dynamic prompt rendering with context

**Prompt Monitoring:**
- **Real-time dashboards**: Track prompt performance metrics
- **Cost tracking per prompt/agent**: Monitor token usage and costs
- **Quality metrics**: Evaluate output quality using custom scorers
- **Alerts for degraded performance**: Threshold-based alerting
- **A/B test result tracking**: Compare prompt variants statistically

### Integration Flow

```
Agent Orchestrator → Prompt Library → LLM Gateway → LLM (Claude/OpenAI/Ollama)
                           ↓
                    Prompt Monitoring ← connects to Trace Logging
                           ↓
...
|----------|---------|----------------|
| `agent_system` | System prompts for agents | All agents |
| `agent_task` | Task-specific prompts | PO refinement, Dev assessment |
| `critique` | Critique and validation | QA INVEST, Developer feasibility |
| `extraction` | Entity/intent extraction | Knowledge retrieval |
| `generation` | Content generation | Story writer |
| `synthesis` | Summarization | PO synthesis |
| `routing` | Orchestrator routing | Supervisor decisions |

### Implementation Details

Key files for prompt management:
- **Schema**: `src/domain/schema.py` (PromptTemplate, PromptVersion, etc.)
- **Interface**: `src/domain/interfaces.py` (IPromptLibrary protocol)
- **Implementation**: `src/infrastructure/prompt_library.py`
- **Monitoring**: `src/utils/prompt_monitor.py`
- **LLM Integration**: `src/adapters/llm/litellm_adapter.py`
...
- Prompt Library: `src/infrastructure/prompt_library.py`
